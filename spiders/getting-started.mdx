---
title: Getting Started with Spiders
description: Learn how to build concurrent, multi-page crawlers with Scrapling's spider system
---
...
2. You've completed or read the [Main classes](/api/selector) page to understand the [Selector](/api/selector) and [Response](/fetching/choosing-fetcher#response-object) classes.
3. You've read the [Architecture](/spiders/architecture) page for a high-level overview of how the spider system works.
</Info>
...
The `parse()` method is where the magic happens. You use the same selection methods you'd use with Scrapling's [Selector](/api/selector)/[Response](/fetching/choosing-fetcher#response-object), and `yield` dictionaries to output scraped items.
...
- [Requests & Responses](/spiders/requests-responses) — learn about request priority, deduplication, metadata, and more.
- [Sessions](/spiders/sessions) — use multiple fetcher types (HTTP, browser, stealth) in a single spider.
- [Advanced features](/spiders/advanced-features-features) — concurrency control, pause/resume, streaming, lifecycle hooks, and logging.
